...
class FlashpointAttention(nn.Module):
    ...
    def __init__(
        ...
        # h_pts = self.no_heads * (2*self.no_qk_points + 2*self.no_v_points) * 3
        h_pts = self.no_heads * (2*self.no_qk_points + self.no_v_points) * 3
        self.linear_points = Linear(self.c_s, h_pts, bias=self.ipa_bias)
        # self.linear_q_pos = Linear(self.c_s, self.no_qk_points * self.no_heads, bias=self.ipa_bias)
        # self.linear_k_pos = Linear(self.c_s, self.no_qk_points * self.no_heads, bias=self.ipa_bias)
        ...
        # self.rel_pos_dim = 0 # TODO: param
        # self.rel_pos_dim = 8 # TODO: param
        # self.rel_pos_dim = 64 # TODO: param
        # self.pos_bias = nn.Parameter(torch.zeros((no_heads, self.rel_pos_dim)))
        # self.flex_compiled = torch.compile(flex_attention)
        self.rotary_emb = RotaryEmbedding(dim=c_hidden)
        self.use_spectra = use_spectra
        ipa_point_weights_init_(self.head_weights)
        # self.dist_scale = nn.Parameter(torch.zeros((no_heads)))
        # self.dist_scale = torch.arange((no_heads)) / 2.0
    ...
    def forward(
        ...
        # [*, N_res, H, P_q/P_v, 3]
        # q_pts, k_pts, qv_pts, kv_pts = torch.split(
        ...
        if res_idx is None:
            res_idx = torch.arange(kv.shape[1], device=kv.device)[None,:].expand(kv.shape[:2])
        if self.use_spectra:
        ...
        # [*, N_res, H, C_hidden]
        # top_z = torch.gather()
        # o, o_pt = torch.split(o, [16, 3*8], dim=-1) # TODO: params
        if self.use_spectra:
